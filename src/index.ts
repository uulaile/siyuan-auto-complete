import { Plugin, fetchSyncPost, Dialog, Protyle } from "siyuan"; //导入
import "@/index.scss";
...
class FootnoteDialog {
  ...
  constructor(
    ...
    let i18n: typeof this.I18N.zh_CN =
      window.siyuan.config.lang in this.I18N
        ? this.I18N[window.siyuan.config.lang]
        : this.I18N.en_US;
    ...
    // Initialize Protyle
    const protyleContainer = this.dialog.querySelector(
      "#footnote-protyle-container"
    );
    this.protyle = new Protyle(
      window.siyuan.ws.app,
      protyleContainer as HTMLElement,
      {
        blockId: blockId,
        mode: "wysiwyg",
        action: ["cb-get-focus"],
  ...
  private drag(e: MouseEvent) {
    ...
    this.currentX = Math.min(
      Math.max(0, this.currentX),
      viewportWidth - rect.width
    );
    this.currentY = Math.min(
      Math.max(0, this.currentY),
...
`,
  ...
  <path d="M30.58,4.85v21.53h-6.57v-2.53h3.05V7.36h-3.05v-2.51h6.57Z" />
    ...
    this.settingUtils.addItem({
      ...
      description:
        this.i18n.settings.reset?.description ||
  ...
  private deleteMemo = ({ detail }: any) => {
    if (detail.element && detail.element.getAttribute("custom-footnote")) {
      detail.menu.addItem({
        ...
        click: async () => {
          const footnote_content_id =
            detail.element.getAttribute("custom-footnote");
          ...
          while (currentElement.previousElementSibling) {
            ...
            if (
              ...
              prevElement
                ...
                ?.includes("custom-footnote-selected-text")
            ) {
              ...
              const newDataType = currentDataType
                ...
                .replace(
                  new RegExp(
                    `\\s*custom-footnote-selected-text-${footnote_content_id}\\s*`,
                    "g"
                  ),
                  ""
                )
  ...
  private async addMemoBlock(protyle: IProtyle) {
    ...
    // 获取当前文档标题
    let currentDoc = await sql(
      `SELECT * FROM blocks WHERE id = '${protyle.block.rootID}' LIMIT 1`
    );
    ...
    switch (this.settingUtils.get("saveLocation")) {
      case "1":
        footnoteContainerTitle = this.settingUtils
          .get("footnoteContainerTitle")
          .replace(/\$\{filename\}/g, currentDocTitle);
    ...
    switch (this.settingUtils.get("saveLocation")) {
      ...
      case "1": // 当前文档
        ...
        if (query_res.length === 0) {
          footnoteContainerID = (
            await appendBlock("markdown", `${footnoteContainerTitle}`, docID)
          )[0].doOperations[0].id;
        } else {
      ...
      case "3": // 子文档
        ...
        if (existingDoc?.length > 0) {
          ...
          docID = await createDocWithMd(
            notebook,
            `${parentPath}/${footnoteContainerTitle}`,
            ""
          );
    ...
    let customFootnotePattern =
      /<span[^>]*?custom-footnote=[^>]*?>.*?<\/span>/g;

    // 正则表达式匹配 <span class="katex">...</span> 及其内容
    let katexPattern =
      /<span class="katex">[\s\S]*?<\/span>(<\/span>)*<\/span>/g;

    // 正则表达式匹配并替换 data-type 中的 custom-footnote-selected-text（包含可能的空格）
    let selectedTextPattern =
      /\s*custom-footnote-selected-text(?:|-[^"\s>]*)(?="|>|\s)/g;
    let selectedTextPattern2 =
      /\s*custom-footnote-hidden-selected-text(?:|-[^"\s>]*)(?="|>|\s)/g;
    ...
    templates = templates.replace(
      /\$\{index\}/g,
      `<span data-type="custom-footnote-index a" data-href="siyuan://blocks/${currentBlockId}">${this.i18n.indexAnchor}</span>`
    ); // 支持添加脚注编号
    templates = templates.replace(
      /\$\{index:text\}/g,
      `<span data-type="custom-footnote-index>${this.i18n.indexAnchor}</span>`
    ); // 支持添加脚注编号
      ...
      switch (this.settingUtils.get("order")) {
        case "2":
          ...
          if (this.settingUtils.get("saveLocation") != 3) {
            back = await appendBlock(
              "markdown",
              templates,
              footnoteContainerID
            );
          } else {
            back = await prependBlock(
              "markdown",
              templates,
              footnoteContainerID
            );
        ...
        default:
          if (this.settingUtils.get("saveLocation") != 3) {
            ...
            const doc = parser.parseFromString(
              footnoteContainerDom,
              "text/html"
            );

            // 查找所有符合条件的div元素
            const footnotes = doc.querySelectorAll(
              `div[custom-plugin-footnote-content="${protyle.block.rootID}"]`
            );
    ...
    await setBlockAttrs(newBlockId, {
      "custom-plugin-footnote-content": protyle.block.rootID,
    });
    await setBlockAttrs(newBlockId, {
      alias: this.settingUtils.get("footnoteAlias"),
    });
    ...
    if (this.settingUtils.get("selectFontStyle") === "2") {
      protyle.toolbar.setInlineMark(
        protyle,
        `custom-footnote-selected-text-${newBlockId}`,
        "range"
      );
    } else {
      protyle.toolbar.setInlineMark(
        protyle,
        `custom-footnote-hidden-selected-text-${newBlockId}`,
        "range"
      );
    }
      ...
      case "2":
        ...
        protyle.toolbar.setInlineMark(protyle, "a sup", "range", {
          ...
          color: `${
            ...
            newBlockId +
            zeroWhite +
            this.settingUtils.get("footnoteBlockref")
          }`,
        });
        memoELement = protyle.element.querySelector(
          `span[data-href="siyuan://blocks/${newBlockId}"]`
        );
        break;
      default:
        // 插入块引
        ...
        new FootnoteDialog2(
          ...
          async (content) => {
            ...
            if (originDOM) {
              ...
              const match = originDOM.match(
                /<span data-type=".*?custom-footnote-index[^>]*>\[(\d+)\]<\/span>/
              );
              if (match) {
            ...
            const templates = this.settingUtils
              .get("templates")
              ...
              .replace(
                /\$\{index\}/g,
                `<span data-type="custom-footnote-index a" data-href="siyuan://blocks/${currentBlockId}">[${number}]</span>`
              ) // 支持添加脚注编号
              .replace(
                /\$\{index:text\}/g,
                `<span data-type="custom-footnote-index">[${number}]</span>`
              ); // 支持添加脚注编号
            ...
            if (existingAttrs) {
              await setBlockAttrs(newBlockId, {
                "custom-plugin-footnote-content":
                  existingAttrs["custom-plugin-footnote-content"],
  ...
  private async reorderFootnotes(
    docID: string,
    ...
    if (footnoteContainerDocID !== docID) {
      ...
      if (!targetDoc) return;
      footnoteContainerDom = new DOMParser().parseFromString(
        targetDoc.content,
        "text/html"
      );
    }
    ...
    const footnoteBlocks = Array.from(
      footnoteContainerDom.querySelectorAll(
        `[custom-plugin-footnote-content="${docID}"]`
      )
    );
    if (footnoteBlocks.length > 0) {
      footnoteBlocks.forEach((block) => {
        ...
        if (blockId) {
          ...
          if (number) {
            // 查找并更新块内的索引编号元素
            const indexSpan = block.querySelector(
              'span[data-type*="custom-footnote-index"]'
            );
            if (indexSpan) {
    ...
    if (footnoteContainerDocID !== docID) {
      await updateBlock(
        "dom",
        footnoteContainerDom.body.innerHTML,
        footnoteContainerDocID
      );
    }
  }
...
export function saveViaTransaction(protyleElem) {
  ...
  if (protyle === null)
    protyle = document.querySelector(
